...
int main() {
    ...
    int next_id = n;
    vector<pair<int,longlong>> events;
    vector<longlong> personal(n+1, 0);
    ...
    for (int _ = 0; _ < m; _++) {
        ...
        if (t == 1) {
            ...
            int x; longlong c; cin >> x >> c;
            int u = findp(x, p);
            int gid = grp[u];
            longlong share = c / sz[u];
            personal[x] += c;
    ...
    vector<int> in(next_id+2), out(next_id+2);
    int timer = 1;
    ...
    vector<longlong> diff(timer+2, 0);
    for (auto &e : events) {
        int id = e.first;
        longlong s = e.second;
        diff[in[id]]   -= s;
    ...
    for (int i = 1; i <= timer; i++) diff[i] += diff[i-1];

    vector<pair<longlong,int>> pos, neg;
    for (int i = 1; i <= n; i++) {
        longlong net = personal[i] + diff[in[i]];
        if (net > 0) pos.emplace_back(net, i);
        else if (net < 0) neg.emplace_back(-net, i);
    }

    vector<tuple<int,int,longlong>> ops;
    int i = 0, j = 0;
    while (i < (int)neg.size() && j < (int)pos.size()) {
        longlong d = min(neg[i].first, pos[j].first);
